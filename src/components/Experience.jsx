...
const ExperienceItem = ({ exp, index, total }) => {
  ...
  return (
    ...
    >
      ...
      {/* Timeline dot */}
      <div className="absolute left-0 md:left-1/2 top-8 w-5 h-5 rounded-full bg-[#E37378] border-4 border-white dark:border-gray-800 shadow-lg transform -translate-x-1/2 z-10 group-hover:scale-125 transition-transform duration-300"></div>

      {/* Content */}
      <div
        className={`bg-white dark:bg-gray-900 p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 ml-8 md:ml-0 md:w-5/12 relative group-hover:-translate-y-1 ${
      ...
      >
        ...
        {/* Date badge */}
        <div className="absolute -top-3 left-6 bg-[#E37378] text-white text-sm font-medium px-3 py-1 rounded-full shadow-md">
          {exp.period}
        </div>

        <h3 className="text-xl font-bold text-[#D94A4F] dark:text-[#ED9C9F] mb-2">
          {exp.role}
        </h3>
        ...
        {/* Hover effect line */}
        <div className="absolute bottom-0 left-0 w-0 h-1 bg-[#E37378] group-hover:w-full transition-all duration-500 ease-out"></div>
      </div>
    </motion.div>
  );
};
